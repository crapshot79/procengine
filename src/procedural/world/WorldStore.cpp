...
void WorldStore::save(const ChunkCoord& cc) {
    ensureDir();
    auto it = removals_.find(cc);
    if (it == removals_.end() || it->second.empty()) return;
...
void WorldStore::load(const ChunkCoord& cc) {
    ...
    if (std::string(line).find("PENGINE_DELTA_V1") == std::string::npos) { std::fclose(f); return; }
    ...
    if (fileSeed == worldSeed_ && fileCC == cc && !keys.empty()) {
        removals_[cc] = std::move(keys);
    }
}
